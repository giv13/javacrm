...
public class ExceptionHandlerAdvice {
    ...
    private ResponseEntity<Response> onHttpMessageNotReadableException(Exception exception) {
        return Response.er(exception.getMessage(), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(AuthenticationException.class)
    private ResponseEntity<Response> onAuthenticationException(Exception exception) {
        return Response.er(exception.getMessage(), HttpStatus.UNAUTHORIZED);
    }

    @ExceptionHandler(InsufficientAuthenticationException.class)
    private ResponseEntity<Response> onInsufficientAuthenticationException() {
        return Response.er("Для доступа к этому ресурсу требуется аутентификация", HttpStatus.UNAUTHORIZED);
    }

    @ExceptionHandler(JwtException.class)
    private ResponseEntity<Response> onJwtException(Exception exception) {
        return Response.er("Просроченный или недействительный JWT", HttpStatus.UNAUTHORIZED);
    }

    @ExceptionHandler(AuthorizationDeniedException.class)
    private ResponseEntity<Response> onAuthorizationDeniedException() {
        return Response.er("Доступ запрещен", HttpStatus.FORBIDDEN);
    }

    @ExceptionHandler(HttpRequestMethodNotSupportedException.class)
    private ResponseEntity<Response> onHttpRequestMethodNotSupportedException(Exception exception) {
    ...
    public ResponseEntity<Response> onMethodArgumentNotValidException(MethodArgumentNotValidException exception) {
    ...
    public ResponseEntity<Response> onConstraintViolationException(ConstraintViolationException exception) {
        Map<String, String> errors = new HashMap<>();
    ...
    private ResponseEntity<Response> onException(Exception exception) {
