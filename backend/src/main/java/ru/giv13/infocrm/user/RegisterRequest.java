...
public record RegisterRequest(
        @NotBlank(groups = FirstValidationGroup.class) @Pattern(regexp = "^\\w+$", message = "Должно содержать только латинские буквы, цифры и символ _", groups = SecondValidationGroup.class) String username,
        @NotBlank(groups = FirstValidationGroup.class) @Email(groups = SecondValidationGroup.class) String email,
        @NotBlank(groups = FirstValidationGroup.class) String name,
